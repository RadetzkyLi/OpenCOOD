...
class IntermediateFusionDataset(basedataset.BaseDataset):
    ...
    def __getitem__(self, idx):
        ...
        for cav_id, selected_cav_base in base_data_dict.items():
            ...
            infra.append(1 if self.is_rsu_agent(cav_id, idx) else 0)
    ...
    def is_rsu_agent(self, agent, idx):
        """
        ...
        """
        _,cur_agent_config,_,_ = self.decode_data_index(idx)
        if agent in cur_agent_config['cav_list']:
            return False
        else:
            return True
    
    @staticmethod
    def get_scalar_velocity(vector_velocity):
        """
        Return the scalar velocity.

        Parameters
        ----------
        vector_velocity: 1d array-like
            E.g., [1.0, 0.1]
        ...
        """
        v = (vector_velocity[0]**2 + vector_velocity[1]**2)**0.5
        return v
...
if __name__ == "__main__":
    ...
    ds = IntermediateFusionDataset(cfg, visualize=False, partname="test")

    # retrive data
    print("dataset length: ", ds.__len__())
